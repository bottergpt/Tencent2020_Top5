...
# 使用的输入
EMB_keys2do = [
    'creative_id', 'product_id', 'ad_id', 'advertiser_id', 'industry',
    'product_category'
]
...
class MultiHeadAttention(keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        queries_multi_heads = tf.concat(tf.split(queries_linear,
                                                 self._n_heads,
                                                 axis=2),
                                        axis=0)
        keys_multi_heads = tf.concat(tf.split(keys_linear,
                                              self._n_heads,
                                              axis=2),
                                     axis=0)
        values_multi_heads = tf.concat(tf.split(values_linear,
                                                self._n_heads,
                                                axis=2),
                                       axis=0)
...
class PositionWiseFeedForward(keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.weights_inner = self.add_weight(shape=(input_shape[-1],
                                                    self._inner_dim),
                                             initializer='glorot_uniform',
                                             trainable=self._trainable,
                                             name="weights_inner")
        self.weights_out = self.add_weight(shape=(self._inner_dim,
                                                  self._model_dim),
                                           initializer='glorot_uniform',
                                           trainable=self._trainable,
                                           name="weights_out")
        self.bais_inner = self.add_weight(shape=(self._inner_dim, ),
                                          initializer='uniform',
                                          trainable=self._trainable,
                                          name="bais_inner")
        self.bais_out = self.add_weight(shape=(self._model_dim, ),
                                        initializer='uniform',
                                        trainable=self._trainable,
                                        name="bais_out")
...
def create_model(hidden_unit=512, concated_id_emb_dict=[]):
    ...
    x_pre_conv = keras.layers.Conv1D(filters=256,
                                     kernel_size=1,
                                     padding='same',
                                     activation='relu')(x)
    ...
    # conv&lstm structure
    x = keras.layers.Conv1D(filters=x.shape[-1].value,
                            kernel_size=1,
                            padding='same',
                            activation='relu')(x)
    conv = keras.layers.Conv1D(filters=256,
                               kernel_size=1,
                               padding='same',
                               activation='relu')(x)
