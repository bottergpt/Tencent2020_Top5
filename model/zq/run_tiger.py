...
from keras.layers import CuDNNLSTM, CuDNNGRU, TimeDistributed, SpatialDropout1D
...
# ALL_MERGED_EMB = ['CACHE_EMB_AVG_ZQ_CBOW_MIN_CNT3_hs0_ng15_XWINDOW_10EPOCH.pkl',
#  'CACHE_EMB_AVG_ZQ_CBOW_MIN_CNT3_hs0_ng15_XWINDOW_20EPOCH.pkl',
#  'CACHE_EMB_AVG_ZQ_CBOW_MIN_CNT3_shuffled_hs0_ng15_XWINDOW_12EPOCH.pkl',
#  'CACHE_EMB_AVG_ZQ_DW_DW_CBOW_hs0ng15_path_length50_XWINDOW_10EPOCH.pkl',
#  'CACHE_EMB_AVG_ZQ_SG_MIN_CNT3_hs0_XWINDOW_10EPOCH.pkl',
#  'CACHE_EMB_AVG_ZQ_SG_MIN_CNT5_hs0_XWINDOW_10EPOCH.pkl',
#  'CACHE_EMB_AVG_ZQ_SG_SESSION_BASED_MIN_CNT3_hs0_XWINDOW_10EPOCH.pkl']
...
YOUR_MARKER = f"MERGED_EMB_Tiger"
...
def concat_byid_emb(all_emb_dict):
...
def create_model(rnn_unit, concated_id_emb_dict):
    ...
    for id_nm, emb_matrix in concated_id_emb_dict.items():
        ...
        cov_layer = keras.layers.Conv1D(filters=256,
                                        kernel_size=1,
                                        activation='relu')
        layers2concat.append(cov_layer(x))
    ...
    x0 = SpatialDropout1D(0.1)(x0)
    x = keras.layers.Bidirectional(CuDNNLSTM(rnn_unit,
                                             return_sequences=True))(x0)
    y0 = keras.layers.GlobalMaxPooling1D()(x)
    y_att1 = Attention(GlobalSeqLength)(x0)
    y_att2 = Attention(GlobalSeqLength)(x)
    yc = Capsule(num_capsule=10,
                 dim_capsule=10,
                 routings=8,
                 share_weights=True)(x0)
    yc = keras.layers.Flatten()(yc)
    y1 = keras.layers.Conv1D(filters=128,
                             kernel_size=1,
                             padding='same',
                             activation='relu')(x)
    y1 = keras.layers.GlobalMaxPooling1D()(y1)
    y2 = keras.layers.Conv1D(filters=64,
                             kernel_size=3,
                             padding='same',
                             activation='relu')(x)
    y2 = keras.layers.GlobalMaxPooling1D()(y2)
    y3 = keras.layers.Conv1D(filters=32,
                             kernel_size=5,
                             padding='same',
                             activation='relu')(x)
    y3 = keras.layers.GlobalMaxPooling1D()(y3)
    concat_all = keras.layers.concatenate([y0, y1, y2, y3, y_att1, y_att2, yc])
    # concat_emb_rnn = rnn_conv(concat_emb, unit=rnn_unit)
    ...
    concat_all = keras.layers.Dense(256)(concat_all)
    concat_all = keras.layers.PReLU()(concat_all)
...
if __name__ == "__main__":
    ...
    all_emb_dict_merged = _load_merged_emb(emb_lst=MERGED_EMB_LST)
    all_emb_dict_raw = _load_embs(zq_emb_lst=ZQ_EMB_LST)
    all_emb_dict = {**all_emb_dict_raw, **all_emb_dict_merged}
    CONCATED_ID_EMB_DICT = concat_byid_emb(all_emb_dict)
    ...
    for count, (train_index, test_index) in enumerate(folds):
        ...
        if (CUR_FOLD >= 0) and (CUR_FOLD < N_FOLDS):
            ...
            print(
                f"CUR_FOLD={CUR_FOLD}, Full 10 folds will be trained in this loop..."
            )
        print("FOLD | ", count)
        print("###" * 35)
        do_aug = True
