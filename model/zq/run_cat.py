...
YOUR_MARKER = f"DataAug_CAT14727_SEED{SEED}"
...
# MODEL
# #################################################################################
def rnn_conv(emb_input, unit):

    x0 = keras.layers.Bidirectional(CuDNNLSTM(
        unit, return_sequences=True))(emb_input)
    x = keras.layers.Bidirectional(CuDNNLSTM(unit, return_sequences=True))(x0)
    y0 = keras.layers.GlobalMaxPooling1D()(x)  # Attention(GlobalSeqLength)(x)
    ...
    concat_all = keras.layers.concatenate([y0, y1, y2, y3])  # y0,
    return concat_all
...
def create_model(rnn_unit, concated_id_emb_dict):
    ...
    concat_all = keras.layers.Dense(512)(concat_emb_rnn)
    concat_all = keras.layers.PReLU()(concat_all)
    concat_all = keras.layers.Dropout(0.5)(concat_all)
