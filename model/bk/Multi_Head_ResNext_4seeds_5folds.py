...
Kernel_Sizes_List = [1, 3, 5, 7]  #四路ResNext的kernel size
...
class Tencent2020:
    def __init__(self, emb_layer, train_data, val_data, test_data, folds, seed,
                 is_resume):
        # Model
    ...
    def test(self, loader, epoch):
        ...
        if not os.path.isdir('../../oof/bk_oof/Multi_Head_ResNext_4seeds_all'):
            os.mkdir('../../oof/bk_oof/Multi_Head_ResNext_4seeds_all')

        pickle.dump(
            output_data,
            open(
                '../../oof/bk_oof/Multi_Head_ResNext_4seeds_all/seed_{}_test_{}_folds_{}.pkl'
                .format(self.seed, epoch, self.folds), 'wb'))
...
if __name__ == '__main__':
    ...
    for seed in [34, 2020, 1111, 200]:
        for folds in range(5):
            ...
            del train_df, val_df, test_df, data
            _ = gc.collect()
