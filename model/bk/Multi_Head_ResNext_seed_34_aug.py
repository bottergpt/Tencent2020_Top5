...
class Tencent2020:
    def __init__(self, emb_layer, train_data, val_data, test_data,
                 test_data_aug, folds, seed, is_resume):
        # Model
    ...
    def train(self):
        ...
        for epoch in range(start_epoch + 1, EPOCHS):
            ...
            if self.val_data is not None:  # Do Validation
                ...
                print('evaluate done!')
                if valid_score > 0.492:
                    self.test(self.test_data, epoch, name='test')
                    self.test(self.test_data_aug, epoch, name='test_aug')
    ...
    def evaluate(self, loader, epoch):
        ...
        if acc_score > 0.48:
            ...
            pickle.dump(
                output_data,
                open(
                    '../../oof/bk_oof/Multi_Head_ResNext_seed_{}_aug/val_{}_folds_{}.pkl'
...
def data_aug(data_df):
    col_list = [
        'time_list', 'creative_id_list', 'ad_id_list', 'product_id_list',
        'advertiser_id_list', 'product_category_list', 'industry_list'
    ]
    data_df_aug = copy.deepcopy(data_df)
    for index in tqdm(range(len(data_df))):
        for col in col_list:
            data_df_aug[index][col] = remove_str(data_df_aug[index][col])
            data_df_aug[index][col].reverse()
            data_df_aug[index][col] = str(data_df_aug[index][col])

    return data_df_aug


#用于合并测试集的增强预测
def test_oof_means(path, name1, name2):
    oof1 = pickle.load(open(path + '{}.pkl'.format(name1), 'rb'))
    oof2 = pickle.load(open(path + '{}.pkl'.format(name2), 'rb'))

    oof1['pred'] = oof1['pred'].apply(
        lambda x: F.softmax(torch.from_numpy(x), dim=0))
    oof2['pred'] = oof2['pred'].apply(
        lambda x: F.softmax(torch.from_numpy(x), dim=0))

    oof_mean = oof1[["user_id"]]
    oof_mean['pred'] = (oof1['pred'] + oof2['pred']) / 2
    pickle.dump(oof_mean, open(path + '{}.pkl'.format(name1), 'wb'))
...
if __name__ == '__main__':
    ...
    for folds in range(5):
        ...
        train_df_aug = data_aug(train_df)
        train_df = train_df + train_df_aug

        test_df_aug = data_aug(test_df)

        dataset_train, dataset_valid, dataset_test, dataset_test_aug = TenCentDataset(
            input_data=train_df), TenCentDataset(
                input_data=val_df), TenCentDataset(
                    input_data=test_df), TenCentDataset(input_data=test_df_aug)

        del train_df, val_df, test_df, data, train_df_aug, test_df_aug
        _ = gc.collect()
        ...
        test_data_aug = DataLoader(dataset_test_aug,
                                   batch_size=BATCH_SIZE * len(device_ids),
                                   shuffle=False,
                                   drop_last=False,
                                   num_workers=2,
        ...
        My_model = Tencent2020(emb_layer, train_data, val_data, test_data,
                               test_data_aug, folds, seed, is_resume)  #实例化模型

        My_model.train()

        #用于对测试集的增强预测（增强数据和原始数据取平均）
        test_oof_means(path='../../oof/bk_oof/Multi_Head_ResNext_seed_34_aug/',
                       name1='test_6_folds_1',
                       name2='test_aug_6_folds_1')
        test_oof_means(path='../../oof/bk_oof/Multi_Head_ResNext_seed_34_aug/',
                       name1='test_6_folds_2',
                       name2='test_aug_6_folds_2')
        test_oof_means(path='../../oof/bk_oof/Multi_Head_ResNext_seed_34_aug/',
                       name1='test_7_folds_3',
                       name2='test_aug_7_folds_3')
        test_oof_means(path='../../oof/bk_oof/Multi_Head_ResNext_seed_34_aug/',
                       name1='test_6_folds_4',
                       name2='test_aug_6_folds_4')
        test_oof_means(path='../../oof/bk_oof/Multi_Head_ResNext_seed_34_aug/',
                       name1='test_8_folds_0',
                       name2='test_aug_8_folds_0')
